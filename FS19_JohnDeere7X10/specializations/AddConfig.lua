...
History:	v1.0 @ 28.02.2017 - initial implementation in FS 17 - added possibility to change capacity via configuration --## FS17 (removed)
			...
			--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			v6.9 @ 25.11.2019 - added the TAG 'getRimColorFromConfig' to get the color for the rims from given config --##FS19
]]
				...
				if #beaconLightIndices > 0 then
					for _, beaconLightIndice in pairs(beaconLightIndices) do
						...
						if hasXMLProperty(self.xmlFile, beaconLightKey) then
							...
							printDebug("(extraOptions) :: Set beacon light(" .. tostring(beaconLightIndice) .. ") filename to '" .. beaconlightsFilename .. "'.", 1, true);
					...
					while true do
						...
						printDebug("(extraOptions) :: Set beacon light(" .. tostring(beaconLightNumber + 1) .. ") filename to '" .. beaconlightsFilename .. "'.", 1, true);
				...
				if colorName ~= nil then
					for _, wheel in pairs(self.spec_wheels.wheels) do
						if wheel.wheelAdditional ~= nil then
							...
							printDebug("(colorChanges) :: Change additional wheel color successfully in '" .. self.configFileName .. "'.", 0, true);
						end;
					end;
				end;
			end;
				...
				if materialId == nil then
					...
					if material ~= nil then	
						...
						if shaderParameter ~= nil then
							...
							printError("(colorChanges) :: Can't find the shader parameter '" .. shaderParameterName .. "' for '" .. materialName .. "'! Set material to '" .. materialId .. "'!", false, true);
						end;
					end;
				end;
		...
		if specCrawlers ~= nil then
			...
			if crawlerColorStr ~= nil then
				...
				for _, crawler in pairs(specCrawlers.crawlers) do
					...
					printDebug("(colorChanges) :: Change crawler color (colorMat0) successfully in '" .. self.configFileName .. "' to '" .. crawlerColorStr .. "' for '" .. getName(crawler.loadedCrawler) .. "'.", 1, true);
					printDebug("(colorChanges) :: Change crawler color (colorMat1) successfully in '" .. self.configFileName .. "' to '" .. Utils.getNoNil(crawlerColor2Str, crawlerColorStr) .. "' for '" .. getName(crawler.loadedCrawler) .. "'.", 1, true);
				...
				for _, attacherJointIndice in pairs(attacherJointIndices) do
					...
					if hasXMLProperty(self.xmlFile, baseName) then
						...
						if baseNode ~= nil then
							...
							printDebug("(colorChanges) :: Change top arm color (colorMat0) successfully in '" .. self.configFileName .. "' to '" .. topArmColorStr .. "' for '" .. getName(baseNode) .. "'.", 1, true);
							printDebug("(colorChanges) :: Change top arm color (colorMat1) successfully in '" .. self.configFileName .. "' to '" .. Utils.getNoNil(topArmColor2Str, topArmColorStr) .. "' for '" .. getName(baseNode) .. "'.", 1, true);

							if topArmColor3 ~= nil then
								I3DUtil.setShaderParameterRec(baseNode, "colorMat2", topArmColor3[1], topArmColor3[2], topArmColor3[3], Utils.getNoNil(materialId, Utils.getNoNil(topArmColor3[4], 0)));
								
								printDebug("(colorChanges) :: Change top arm decal color (colorMat2) successfully in '" .. self.configFileName .. "' to '" .. topArmColor3Str .. "' for '" .. getName(baseNode) .. "'.", 1, true);
							end;
						end;
					end;
				end;
			...
			if starfire ~= nil then
				...
				if starfireColorStr ~= nil then
					...
					if starfireColor ~= nil then	
						...
						printDebug("(colorChanges) :: Change starfire color (colorMat0) successfully in '" .. self.configFileName .. "' to '" .. starfireColorStr .. "' for '" .. getName(starfire) .. "'.", 1, true);
					else
					...
					if starfireColor2 ~= nil then
						...
						printDebug("(colorChanges) :: Change starfire color (colorMat1) successfully in '" .. self.configFileName .. "' to '" .. Utils.getNoNil(starfireColor2Str, starfireColorStr) .. "' for '" .. getName(starfire) .. "'.", 1, true);
					else
